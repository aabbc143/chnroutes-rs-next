...
///
/// When network request fails, the built-in APNIC snapshot is used as fallback.
/// The fallback data is deliberately not written back to cache.
    ...
    match client.get(APNIC_URL).send() {
        ...
        Ok(response) => match response.error_for_status() {
            ...
            Err(error) => {
                ...
                warn!("APNIC returned unsuccessful HTTP status: {error}");
...
/// Load embedded APNIC snapshot.
///
/// Used only as fallback and never written into normal cache.
fn load_builtin_data() -> crate::error::Result<Vec<IpNet>> {
    ...
    let compressed_bytes =
        include_bytes!(concat!(env!("OUT_DIR"), "/apnic.zst"));
...
/// Parse APNIC delegated statistics.
///
